"""
Standalone object detection module for scene graph construction.

Runs OUTSIDE the SLAM tracking loop. Consumes saved keyframes from DROID-W
and produces per-frame detections (bounding boxes, labels, confidences).
Nothing flows back into SLAM — the tracking pipeline doesn't know this exists.

Data flow:
    DROID-W saves keyframes (RGB, pose, depth, FiT3D features, uncertainty)
    → This module reads RGB, runs YOLO-World, saves detections
    → Scene graph module combines detections + DROID-W outputs
"""
