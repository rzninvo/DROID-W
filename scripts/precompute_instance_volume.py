...
pixels via depth + pose to world XYZ, and concatenate. Stores subsampled
(stride configurable) per-track point clouds in a ragged structure.
...
Output (<panoptic.parent>/instance_volumes.npz):
    points          : (P_total, 3) float32  — world XYZ, all tracks concatenated.
    track_offsets   : (G+1,)       int64    — points[track_offsets[g]:track_offsets[g+1]] is track g.
    track_id        : (G,)         int64    — same as global_track_id; for cross-ref.
    track_n_points  : (G,)         int64    — bookkeeping.
    voxel_size      : float                 — 0.05 m for OVI-MAP-style downstream.
    scene           : str                   — config path.

Verification gate (Plan B B.3):
  - 1-5 M total points per Replica scene.
  - 50-5000 per-instance mean.
  - 1% sample re-projects into source KF within 3 px.
...
def main() -> int:
    ...
    # ── Pass 1: accumulate points per track ────────────────────────────────
    ...
    # ── Pass 2: pack into ragged arrays + cap per-track ────────────────────
